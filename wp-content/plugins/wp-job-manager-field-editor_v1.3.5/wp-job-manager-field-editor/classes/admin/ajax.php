...
class WP_Job_Manager_Field_Editor_Ajax extends WP_Job_Manager_Field_Editor_Admin {
			...
			if( ! empty( $tmp_options ) || ! empty( $allowed_mimes ) ) {
				...
				foreach( $tmp_options as $ext => $type ){
					...
					if( ! in_array( $type, $allowed_mimes ) ){
						...
						$cache_break = "AB80YWCz31M03JJ/ciBZogEE";
			...
			switch ( $this->post_modal_action ) {
				...
				case "new":
					...
					if ( $this->custom_field_exists() ) {
						...
						$this->is_error = true;
						$response[ 'message' ] = __( 'Sorry, that meta key already exists, please choose another or edit the existing one.', 'wp-job-manager-field-editor' );
	...
	private function custom_field_exists() {
		...
		if ( empty( $check_fields ) ) return FALSE;

		return array_key_exists( $this->post_meta_key, $check_fields );
	}
