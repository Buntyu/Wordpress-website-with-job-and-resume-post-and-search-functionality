...
class WP_Resume_Manager_Form_Edit_Resume extends WP_Resume_Manager_Form_Submit_Resume {
	...
	public function submit() {
		...
		$resume = get_post( $this->resume_id );
                // Changes by Gaurav (dated 7th Oct 2016) on line 52. Added condition for pending as well so that resume can be edtied for pending approval resumes. 
		if ( empty( $this->resume_id  ) || ( $resume->post_status !== 'publish' && $resume->post_status !== 'hidden' && $resume->post_status !== 'pending' ) ) {
	...
	public function submit_handler() {
		...
		try {
			...
			// Update the resume
                        // Changes by Gaurav (dated 7th Oct 2016) from line - 116 - 118
			...
			//$this->save_resume( $values['resume_fields']['candidate_name'], $values['resume_fields']['resume_content'], 'publish', $values );
                        $this->save_resume( $values['resume_fields']['candidate_name'], $values['resume_fields']['resume_content'], $post_status, $values );
