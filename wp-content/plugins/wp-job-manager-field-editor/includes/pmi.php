...
if( ! class_exists( 'Post_Meta_Inspector' ) ){
	...
	class Post_Meta_Inspector {
		...
		public function action_add_meta_boxes() {
			...
			add_meta_box( 'post-meta-inspector', __( 'Post Meta Inspector', 'wp-job-manager-field-editor', 'wp-job-manager-field-editor' ), array(self::$instance, 'post_meta_inspector'), get_post_type() );
		}
		...
		public function post_meta_inspector() {
			...
			$toggle_el     = '<a href="javascript:void(0);" class="pmi_toggle">' . __( 'Click to show&hellip;', 'wp-job-manager-field-editor', 'wp-job-manager-field-editor' ) . '</a>';
			?>
			<style>
		...
		<thead>
			<tr>
				<th class="key-column"><?php _e( 'Key', 'wp-job-manager-field-editor', 'wp-job-manager-field-editor' ); ?></th>
				<th class="value-column"><?php _e( 'Value', 'wp-job-manager-field-editor', 'wp-job-manager-field-editor' ); ?></th>
			</tr>
		</thead>
		<tbody>
	<?php foreach( $custom_fields as $key => $values ) :
		if( apply_filters( 'pmi_ignore_post_meta_key', FALSE, $key ) )
			continue;
		?><?php foreach( $values as $value ) : ?><?php
		$value   = var_export( $value, TRUE );
		$toggled = $toggle_length && strlen( $value ) > $toggle_length;
		?>
		<tr>
			<td class="key-column"><?php echo esc_html( $key ); ?></td>
			<td class="value-column"><?php if( $toggled ) echo $toggle_el; ?>
				<code <?php if( $toggled ) echo ' style="display: none;"'; ?>><?php echo esc_html( $value ); ?></code></td>
		</tr>
...
if( ! function_exists( 'Post_Meta_Inspector' ) ){
