...
class sMyles_Update {
	...
	// Start Workspace
	private $api_url     = 'http://plugins.smyl.es/?wc-api=upgrade-api';
	...
	public function admin_init() {
		...
		if ( ! empty( $_GET[ $this->plugin_slug . '_deactivate_licence' ] ) ) {
			...
			wp_redirect( admin_url( 'plugins.php?deactivated_licence=' . $this->plugin_slug . '#wpwrap' ) );
			exit;
		}
		...
		if ( ! empty( $_POST[ $this->plugin_slug . '_licence_key' ] ) ) {
			...
			wp_redirect( admin_url( 'edit.php?post_type=job_listing&page=field-editor-settings#settings-license' ) );
			exit;
		}
